...
app.post('/api/admin/ask-openai', verificarToken, verificarAdmin, async (req, res) => {
    ...
    try {
		...
		### REGLAS CRÍTICAS E INQUEBRANABLES:
    ...
    try {
        ...
        for (const barbero of barberos) {
            const pagos = await Pago.findAll({
                where: {
                    barberia_id: req.user.barberia_id,
                    fecha_pago: { [Op.between]: [`${fechaInicio} 00:00:00`, `${fechaFin} 23:59:59`] }
                },
                include: [{
                    model: Cita,
                    required: true,
                    where: { barbero_id: barbero.barbero_id },
                    include: [
                        { model: Cliente, attributes: ['nombre', 'apellido'] },
                        { model: Servicio, attributes: ['nombre', 'precio'], through: { attributes: [] } }
                    ]
                }],
            ...
            let totalIngresos = 0;
            const detallesServicios = []; // Array para cada servicio individual

            pagos.forEach(pago => {
                totalIngresos += parseFloat(pago.monto_total);
                pago.Cita.Servicios.forEach(servicio => {
                    let valorComision = 0;
                    // Calcula la comisión solo si el tipo de pago es 'comision'
                    if (barbero.tipo_pago === 'comision' && barbero.valor) {
                        valorComision = parseFloat(servicio.precio) * (barbero.valor / 100);
                    }

                    detallesServicios.push({
                        fecha: new Date(pago.fecha_pago).toLocaleDateString('es-CO'),
                        cliente: `${pago.Cita.Cliente.nombre} ${pago.Cita.Cliente.apellido}`,
                        servicioNombre: servicio.nombre,
                        servicioPrecio: parseFloat(servicio.precio),
                        valorComision: valorComision // <-- El nuevo valor calculado
                    });
                });
            });
            ...
            resultadoLiquidacion.push({
                ...
                pagosDetallados: detallesServicios 
            });
        }
