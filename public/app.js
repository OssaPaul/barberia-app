...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupEventListeners = () => {
        ...
        // =================== CÓDIGO MODIFICADO ===================
        document.getElementById('btn-generar-reporte-liq').addEventListener('click', async () => {
            const barberoId = document.getElementById('liq-barbero-select').value;
            const fechaInicio = document.getElementById('liq-fecha-inicio').value;
            const fechaFin = document.getElementById('liq-fecha-fin').value;
            if (!fechaInicio || !fechaFin) {
                alert('Por favor, seleccione ambas fechas.');
                return;
            }

            const reportBody = document.getElementById('body-reporte-liq');
            reportBody.innerHTML = '<tr><td colspan="4">Generando liquidación...</td></tr>';
            
            const data = await fetchData(`/api/reportes/liquidacion?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}&barberoId=${barberoId}`);
            const exportButtons = document.getElementById('export-buttons-liq');
        
            const footerContainer = document.getElementById('footer-reporte-liq');
            ...
            if (data && data.length > 0) {
                ...
                let granTotalAPagar = 0;

                data.forEach(liq => {
                    granTotalIngresos += liq.ingresosGenerados;
                    granTotalAPagar += liq.valorAPagar;

                    const row = document.createElement('tr');
                    row.classList.add('fila-resumen-liq');
                    row.style.cursor = 'pointer';
                    row.title = 'Haz clic para ver el detalle';
                    
                    row.innerHTML = `
                        <td data-label="Barbero:"><i class="fas fa-chevron-right expand-icon"></i> ${liq.barbero}</td>
                        <td data-label="Ingresos:">${formatCurrency(liq.ingresosGenerados)}</td>
                        <td data-label="% Comisión:">${liq.porcentajeComision} %</td>
                        <td data-label="A Pagar:">${formatCurrency(liq.valorAPagar)}</td>
                    `;

                    const detalleRow = document.createElement('tr');
                    const detalleCell = document.createElement('td');
                    detalleCell.colSpan = 4;
                    detalleCell.classList.add('detalle-liquidacion-cell');

                    let detalleHtml = '<div class="detalle-wrapper"><table><thead><tr><th>Fecha</th><th>Cliente</th><th>Servicio</th><th style="text-align:right;">Valor Comisión</th></tr></thead><tbody>';
                    
                    liq.pagosDetallados.forEach(detalle => {
                        detalleHtml += `
                            <tr>
                                <td>${detalle.fecha}</td>
                                <td>${detalle.cliente}</td>
                                <td>${detalle.servicioNombre} (${formatCurrency(detalle.servicioPrecio)})</td>
                                <td style="text-align:right; font-weight: bold;">${formatCurrency(detalle.valorComision)}</td>
                            </tr>
                        `;
                    });
                    ...
                    reportBody.appendChild(row);
                    reportBody.appendChild(detalleRow);

                    row.addEventListener('click', () => {
                        const icon = row.querySelector('.expand-icon');
                        row.classList.toggle('expanded'); // Clase para el CSS
                ...
                if (footerContainer) {
                    footerContainer.innerHTML = `
                        <tr>
                            <td data-label="Totales:" style="font-weight: bold;">TOTALES GENERALES</td>
                            <td data-label="Total Ingresos:" style="font-weight: bold;">${formatCurrency(granTotalIngresos)}</td>
                            <td></td>
                            <td data-label="Total a Pagar:" style="font-weight: bold;">${formatCurrency(granTotalAPagar)}</td>
                        </tr>
                    `;
                }
                ...
                reportBody.innerHTML = '<tr><td colspan="4">No se encontraron datos para la liquidación.</td></tr>';
                exportButtons.style.display = 'none';
            }
        });
